...
const validationSchema = Yup.object().shape({
  ...
  comment: Yup.string(),
  phoneFixe: Yup.string().required("Le numéro fixe est requis"),
...
export default function AppointmentAdd({closeModal}) {
  ...
  const onSubmit = async (data) => {
    ...
    const phone = [phoneFixe, phoneMobile];
    
    const updateData = { name, comment, commercial, date, address, phone }
    await dispatch(appointmentsActions.createAppointment({id:authUser?.id, appointment: updateData}));

    console.log(updateData, authUser?.id);
    closeModal();
    reset();
  };
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Modal.Footer>
                <Button disabled={isSubmitting} type="submit">
                  {isSubmitting ? <span className=""></span> : "Save"}
                </Button>
                <Button
                  color="gray"
